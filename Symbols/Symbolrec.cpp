...
string getColour(Mat roiframe)
{
    string colour;
    Mat frame_HSV, frame_GREY, kernel;

    cvtColor(roiframe, frame_HSV, COLORBGR2HSV);
    inRange(frame_HSV, Scalar(139, 115, 115), Scalar(150, 255, 255), frame_GREY);
    kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
    cv::morphologyEx(frame_GREY, frame_GREY, MORPH_OPEN, kernel);

    int numNonZero = 0;
    numNonZero = countNonZero(frame_GREY);

    if(numNonZero > 10000)
    {
        delay(2000);        //adjust this delay for the correct stopping distance
        char dataString[8] = {};
        sprintf(dataString, "0/0/90"); // this is String printf (prints to a string)
        arduino.i2cWrite(dataString, 7);

        Mat Symbol = captureFrame();
        cvtColor(Symbol, Symbol_GREY, COLORBGR2HSV);
        kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
        cv::morphologyEx(Symbol_GREY, Symbol_GREY, MORPH_OPEN, kernel);


        std::vector< std::vector<cv::Point> > contours; // Variable for list of contours std
        findContours(Symbol_GREY, contours, RETR_LIST, CHAIN_APPROX_SIMPLE);

        int i, MaxValue, MaxValueIndex, Area;
        ...
        for(i = 0 ; i < contours.size() ; i++)
        {
            drawContours(Symbol, contours, i, scalar(0, 0, 255))
        }

        polylines(Symbol, contours[MaxValueIndex], true, Scalar(0, 255, 0));
...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        string colour = getColour(roiFrame);
        ...
        char dataString[8] = {};
        sprintf(dataString, "0/0/90"); // this is String printf (prints to a string)
        arduino.i2cWrite(dataString, 7);      // write the string via i2c
