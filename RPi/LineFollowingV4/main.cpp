...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        int correctedAngle = 90 + PID(angle-90,&sumAngle,&lastAngle);//PID!
        cout << correctedAngle << endl;
        ...
        {
            rightSpeed=BASESPEED-((correctedAngle-90)*0.5);
            leftSpeed=BASESPEED+((correctedAngle-90)*0.5);
            if (abs(correctedAngle-90 >= 20))
            {
                rightSpeed = rightSpeed*0.8;
                leftSpeed = leftSpeed*0.8;
            }
            else if (abs(correctedAngle-90) >= 30)
            {
        ...
        sprintf(dataString, "%-3i/%-4i/%-4i", correctedAngle,rightSpeed,leftSpeed);// this is String printf (prints to a string)
        ...
        printf("Gradient: %-8.2fRaw Angle: %-8.2fAngle: %i\tCorrected Angle: %i\tRightSpeed: %-5iLeftSpeed: %-5i\n", gradient, rawAngle, angle, correctedAngle, rightSpeed,leftSpeed);//print results
