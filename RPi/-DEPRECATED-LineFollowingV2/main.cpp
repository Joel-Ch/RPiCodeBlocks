...
int main(int argc, char **argv)
{
    ...
    char* colourArray = &colour[0];
    ESP.i2cWrite(colourArray,6);

    // Initialize ROI for line following
    Rect roi(0, ROI_TOP, FRAME_WIDTH, ROI_BOTTOM - ROI_TOP);

    float lastError = 0, lastAngle=0, sumAngle=0;
    ...
    while (true)
    {
        ...
        else if(colour=="red")
        {
            //two maks for red
            cv::Mat binary2;
            inRange(roi_frame_hsv,Scalar(0,70,50),Scalar(10,255,255),binary);
        ...
        Mat contourDisp = roi_frame.clone();
        ...
        cv::circle(contourDisp,centre,5,Scalar(0,255,0),3);
        ...
        //this function returns an angle from 90 -> -90 but -90 and 90 are both almostvertical so this needs some editing to enable a useful angle output
        float angle;
        if (rawAngle > 0)
        {
            angle=180-rawAngle;
        ...
        float correctedAngle = PID(angle-90,&sumAngle,&lastAngle);

        line(contourDisp,centre,centreBottom,Scalar(0,255,0),2);

        imshow("Contour", contourDisp);




        arduino.i2cWriteArduinoInt((int)angle);
        //cout << "X: " << centre.x<< "\tY: " << centre.y << "\tGradient: " << gradient << "\tRAWANGLE: " << rawAngle <<"\tANGLE: " << angle <<endl;
        printf("X: %i\tY: %i\tGradient: %-8.2fRaw Angle: %-8.2fAngle: %-8.2fCorrected Angle: %-8.2f\n", centre.x, centre.y, gradient, rawAngle, angle, correctedAngle);
        ...
        usleep(25000);//delay (servo can only update at 50Hz so must delay until servo can update again)
    }
